...
export function ResultsPage() {
  ...
  useEffect(() => {
    ...
    const setupListeners = async () => {
      const unlistenFileFound = await listen<RecoveredFile>(
        "file_found",
        (event) => {
          const path = event.payload.original_path;
      ...
      const unlistenScanComplete = await listen<string>("scan_complete", () => {
        setScanning(false);
  ...
  const saveSelectedFiles = async () => {
    try {
      ...
      if (errors > 0) {
        alert(
          `${savedCount} arquivo(s) salvos com sucesso!\n${errors} arquivo(s) com erro.`
        );
      } else {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center">
      <div className="w-[95%] ml-2 flex flex-col gap-6 p-3">
            ...
            <div className="flex items-center gap-6 p-2">
              ...
              <div>
                ...
                <p className="text-gray-400 text-base">
                  Analisando setores do disco em busca de arquivos deletados
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="p-8 rounded-2xl bg-linear-to-r from-green-500/10 via-emerald-500/10 to-teal-500/10 border-2 border-green-500/40">
            <div className="flex items-center gap-8 p-2">
              ...
              <div>
                ...
                <p className="text-gray-400 text-base">
                  {recoveredFiles.length} arquivo(s) recuperado(s) e pronto(s)
                  para download
                </p>
              </div>
            </div>
        ...
        {/* Filters */}
        <div className="flex items-center justify-between gap-6 p-6 rounded-xl h-8 bg-gray-900/60 border-2 border-gray-700/50 backdrop-blur-sm">
          <div className="flex items-center gap-4">
            <span className="text-base font-semibold text-gray-300">
              Categorias:
            </span>

            {["all", "images", "documents", "videos", "audios"].map(
              (category) => (
                <button
                  key={category}
            ...
            <div className="flex flex-col items-center justify-center py-32 space-y-6">
              ...
              {!scanning && (
                <p className="text-gray-500 text-lg">
                  Tente ajustar os filtros ou iniciar uma nova varredura
                </p>
              )}
            </div>
          ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="shrink-0">
                      <div
                        className={`px-8 py-4 rounded-xl bg-linear-to-r ${getCategoryColor(
                          file.category
                        )} text-white font-bold shadow-lg flex items-center justify-center gap-3 min-w-40`}
                      >
        ...
        {selectedFiles.size > 0 && (
          <div className="sticky bottom-5 flex justify-center">
            <div className="flex justify-center rounded-2xl bg-gray-900/95 backdrop-blur-xl border-2 border-violet-500/50 shadow-2xl shadow-violet-500/30 w-80 h-12">
              <button
